...
constexpr uint32_t DEAD_TIME = 1.0*DEAD_TIME_nsX100*PWM_FREQUENCY_Hz*PWM_MAX_VAL
                               / SECOND_nsX100;
...
inline void set_phase_dutycycle(PhaseSelector phase, uint32_t value) {
	...
	ledc_set_duty_and_update(LEDC_HIGH_SPEED_MODE, phase_component_h,
		std::min(dutycycle_h+DEAD_TIME, PWM_MAX_VAL), 0
		// dutycycle_h, 0
	);
	ledc_set_duty_and_update(LEDC_HIGH_SPEED_MODE, phase_component_l,
		(dutycycle_l<DEAD_TIME)?0:(dutycycle_l-DEAD_TIME), DEAD_TIME
		// (dutycycle_l<DEAD_TIME)?0:(dutycycle_l-DEAD_TIME), DEAD_TIME/2
		// dutycycle_l, 0
	);
}
